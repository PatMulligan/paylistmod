...
window.app = Vue.createApp({
  ...
  methods: {
    paylistmodFunction(data) {
      LNbits.api
        .request(
          'GET', // Type of request
          '/paylistmod/api/v1/test/00000000', // URL of the endpoint
          this.g.user.wallets[0].inkey, // Often endpoints require a key
          data
        )
        .then(response => {
          this.paylistmodData.push(mapObject(response.data)) // Often whats returned is mapped onto some model
        })
    ...
    getVettedReadme() {
      ...
      LNbits.api
        .request('GET', '/paylistmod/api/v1/vetted', this.g.user.wallets[0].inkey)
        .then(response => {
